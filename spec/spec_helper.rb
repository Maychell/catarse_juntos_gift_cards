...
RSpec.configure do |config|
  ...
  config.include CatarseJuntosGiftCards::Engine.routes.url_helpers
...
def juntos_gift_cards_url(path, options = {})
  options[:https] = true if options[:https].nil?
  options[:country] ||= 'argentina'
  protocol = (options[:https] == true) ? 'https' : 'http'
  "#{protocol}://#{options[:country]}.juntos_gift_cards.com#{path}"
end

def stub_get(path, filename, options={})
  opts = {:body => fixture_file(filename)}.merge(options)

  FakeWeb.register_uri(:get, juntos_gift_cards_url(path), opts)
end

def stub_post(path, filename, options={})
  opts = {:body => fixture_file(filename)}.merge(options)

  FakeWeb.register_uri(:post, juntos_gift_cards_url(path, options[:url]), opts)
end
